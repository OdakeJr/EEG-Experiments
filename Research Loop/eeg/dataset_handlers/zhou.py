...
def _prepare_zhou_labels(
    ...
    """
    Map Zhou2016 stimulus codes to canonical labels.
    """
...
def load_zhou2016_data(
    ...
    """
    Load Zhou2016 into the standardized intermediate
    EEG representation.
    ...
    if not root.exists():
        raise FileNotFoundError(
            f"Zhou2016 directory not found: "
            f"{root}"
        )
...
def prepare_zhou(params):
    ...
    if root is None:
        raise ValueError(
            "'root' must be specified for Zhou2016."
        )
    ...
    data, info = prepare_eeg_dataframe(
        ...
        # Loader preserves the three real sessions.
        session_name=None,
