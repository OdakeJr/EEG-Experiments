...
def _prepare_zhou_channels(
    ...
    """
    Optionally select a subset of Zhou2016 EEG electrodes.
    ...
    if missing_channels:
        raise ValueError(
            "Requested channels are not available in Zhou2016: "
            f"{missing_channels}. "
...
def _prepare_zhou_labels(
    ...
    if unknown_labels:
        raise ValueError(
            "Unknown Zhou2016 labels found in metadata: "
            f"{unknown_labels}"
        )
    ...
    if unknown_classes:
        raise ValueError(
            "Requested classes are not available in Zhou2016: "
            f"{unknown_classes}. "
...
def load_zhou2016_data(
    ...
    """
    Load Zhou2016 data into a nested dictionary.

    Labels are stored as semantic strings:
    ...
    for subject in subjects:
        ...
        for session_name in sessions:
            ...
            code_to_label = _prepare_zhou_labels(
                metadata=metadata,
            ...
            for event_sample in event_samples:
                ...
                # FII representation stores EEG in microvolts.
