...
def _feature_selection_config_label(fs_params, method, method_params):
    if "feature_selection_config_label" in fs_params:
        return _safe_label(fs_params["feature_selection_config_label"])

    if "config_label" in fs_params:
        return _safe_label(fs_params["config_label"])

    if "name" in fs_params and fs_params["name"] != method:
        return _safe_label(fs_params["name"])

    params_text = _params_label(method_params)
...
def _get_fit_data(data, fit_partitions):
    ...
    if not X_parts:
        raise ValueError("No samples available to fit feature transformation.")
...
def _validate_representation(transformer, representation):
    ...
    if expected != "any" and expected != representation:
        raise ValueError(
            ...
            f"but preprocessing produced '{representation}'."
        )
...
def _make_feature_selection_artifact(
    split, transformer_path, manifest_path, signature,
    method, method_params, config_label, view,
):
    return FeatureSelectionArtifact(
        ...
        feature_selection_params=_json_copy(method_params),
...
def run_feature_selection(split, view, fs_params, group="default"):
    method = fs_params["method"]
    method_params = fs_params.get("params", {})
    config_label = _feature_selection_config_label(fs_params, method, method_params)
    name = fs_params.get("name", config_label)
    fit_partitions = fs_params.get("fit_partitions", ["train"])
    representation = getattr(view, "representation", "features")
    ...
    try:
        ...
        transformer = get_feature_transformer(method, method_params)
        _validate_representation(transformer, representation)
