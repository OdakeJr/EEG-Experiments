...
def _median_sigma(X, seed=42, max_samples=500):
    ...
    values = distances[np.triu_indices(len(X), k=1)]
...
class _KLIEP:
    ...
    def __init__(self, sigma=None, n_centers=100, max_iter=500, seed=42):
        self.sigma = sigma
    ...
    def fit(self, X_source, X_target):
        ...
        indices = rng.choice(len(X_target), size=n_centers, replace=False)
        self.centers = X_target[indices]
        ...
        K_source = _rbf_kernel(X_source, self.centers, self.sigma)
        K_target = _rbf_kernel(X_target, self.centers, self.sigma)

        source_mean = K_source.mean(axis=0)
        alpha0 = np.ones(n_centers)
        alpha0 /= max(source_mean @ alpha0, 1e-12)

        def objective(alpha):
            ratios = K_target @ alpha
            return -np.mean(np.log(ratios + 1e-12))

        def gradient(alpha):
            ratios = K_target @ alpha
            return -np.mean(
                K_target / (ratios[:, None] + 1e-12),
                axis=0,
            )

        constraint = {
            "type": "eq",
            "fun": lambda alpha: source_mean @ alpha - 1.0,
            "jac": lambda alpha: source_mean,
        }
        ...
        result = minimize(
            ...
            alpha0,
            jac=gradient,
            bounds=[(0.0, None) for _ in range(n_centers)],
        ...
        if not result.success:
            raise RuntimeError(
                f"KLIEP optimization failed: {result.message}"
            )
...
class _ULSIF:
    ...
    def fit(self, X_source, X_target):
        ...
        H = (K_source.T @ K_source) / len(X_source)
        h = K_target.mean(axis=0)

        regularized = (
            H
            + self.regularization * np.eye(n_centers)
        )

        self.alpha = np.linalg.solve(regularized, h)
        return self
...
def _get_density_ratio_estimator(name, params, seed):
    name = name.lower()
    params = dict(params)
    params.setdefault("seed", seed)

    if name == "kliep":
        return _KLIEP(**params)

    if name == "ulsif":
        return _ULSIF(**params)

    raise ValueError(
        f"Unknown density-ratio estimator '{name}'. "
...
class ImportanceWeighting(BaseLearningAlgorithm):
    ...
    def fit(
        ...
        **training_params,
        ...
        estimator_name = training_params.get("estimator", "kliep")
        estimator_params = training_params.get("estimator_params", {})
        normalize_weights = training_params.get("normalize_weights", True)
        max_weight = training_params.get("max_weight", None)
        ...
        ratio_estimator = _get_density_ratio_estimator(
            estimator_name,
            estimator_params,
            seed,
        )

        weights = ratio_estimator.fit_predict(
            X_source,
            X_target,
        )
        ...
        if not np.all(np.isfinite(weights)):
            raise ValueError(
                "Importance weights contain non-finite values."
            )
        ...
        y_encoded = np.array(
            [class_to_index[label] for label in y_source]
        )
        ...
        for _ in range(epochs):
            for X_batch, y_batch, weight_batch in loader:
                ...
                loss = (
                    weight_batch * losses
                ).sum() / weight_batch.sum().clamp_min(1e-12)

                loss.backward()
                optimizer.step()

        self.model = model
        self.density_ratio_estimator = ratio_estimator
    ...
    def _get_source_data(source):
        if source is None:
            raise ValueError(
                "Importance weighting requires source data."
            )
