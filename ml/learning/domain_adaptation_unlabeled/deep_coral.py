...
class DeepCORAL(BaseLearningAlgorithm):
    ...
    def fit(
        ...
        **training_params,
        ...
        coral_lambda = training_params.get("coral_lambda", 1.0)
    ...
    def _coral_loss(source, target):
        ...
        source_covariance = DeepCORAL._covariance(source)
        target_covariance = DeepCORAL._covariance(target)
        dimension = source.shape[1]

        return (
            (source_covariance - target_covariance).pow(2).sum()
