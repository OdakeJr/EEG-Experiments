...
def _prepare_zhou_channels(data, channel_names, channels=None):
    ...
    if missing:
        raise ValueError(
            f"Requested channels are not available in Zhou2016: {missing}."
        )
...
def _prepare_zhou_labels(metadata, selected_classes=None):
    yaml_labels = metadata["stim"]["labels"]

    unknown = [
        label for label in yaml_labels
        if label not in ZHOU_LABEL_MAP
    ]
    if unknown:
        raise ValueError(f"Unknown Zhou2016 labels: {unknown}")

    code_to_label = {
        int(code): ZHOU_LABEL_MAP[label]
    ...
    valid = set(ZHOU_LABEL_MAP.values())
    unknown = [label for label in selected_classes if label not in valid]

    if unknown:
        raise ValueError(
            f"Requested classes are not available in Zhou2016: {unknown}."
        )
...
def load_zhou2016_data(root, config=None):
    config = _merge_config(config)
    root = Path(root)

    if not root.exists():
        raise FileNotFoundError(f"Zhou2016 directory not found: {root}")
    ...
    for subject in subjects:
        ...
        for session_name in sessions:
            ...
            data, selected_channels = _prepare_zhou_channels(
                data, channel_names, channels
            )
            code_to_label = _prepare_zhou_labels(metadata, classes)
...
def prepare_zhou(params):
    root = params.get("root")

    if root is None:
        raise ValueError("'root' must be specified for Zhou2016.")
    ...
    return prepare_eeg_dataframe(
        ...
        subjects=subjects,
        subject_batch_size=params.get("subject_batch_size", 2),
